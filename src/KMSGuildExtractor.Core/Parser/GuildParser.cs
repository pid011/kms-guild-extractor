...
namespace KMSGuildExtractor.Core.Parser {
  public static class GuildParser {
    ...
    /// <returns>만약 일치하는 길드가 없거나 잘못된 html문서면 null을
    /// 반환</returns>
    public static GuildInfo FindGuildInHtml(HtmlDocument html, WorldID world) {
      try {
        HtmlNode rankNode = html.DocumentNode.SelectSingleNode(
            "//table[@class=\"rank_table2\"]/tbody");
        ...
        foreach (HtmlNode item in rankNode.SelectNodes(".//tr[@class=\"\"]")) {
          ...
          int guildLevel = ParseTool.GetDigitInString(
    ...
    public static WeeklyGuildReputationInfo
    GetWeeklyReputation(HtmlDocument html) {

      try {
        HtmlNode infoNode = html.DocumentNode.SelectSingleNode(
            "//div[@class=\"char_info_top\"]");
        HtmlNodeCollection weeklyRankNode =
            infoNode.SelectNodes("./div[@class=\"char_info\"]/dl");

        string weeklyOverallRankRaw =
            weeklyRankNode [0]
                .SelectSingleNode("./dd[@class=\"num\"]")
                .InnerText;
        int weeklyOverallRank =
            ParseTool.GetDigitInString(weeklyOverallRankRaw);

        string weeklyWorldRankRaw =
            weeklyRankNode [1]
                .SelectSingleNode("./dd[@class=\"num\"]")
                .InnerText;
        int weeklyWorldRank = ParseTool.GetDigitInString(weeklyWorldRankRaw);

        string weeklyScoreRaw =
            infoNode
                .SelectNodes(".//ul[@class=\"info_tb_left_list\"]/li") [2]
                .GetDirectInnerText();
        IFormatProvider provider = CultureInfo.CreateSpecificCulture("ko-KR");
        int weeklyScore =
            int.TryParse(weeklyScoreRaw,
                         NumberStyles.Number | NumberStyles.Integer, provider,
                         out int result)
                ? result
                : 0;

        return new WeeklyGuildReputationInfo{Score = weeklyScore,
    ...
    public static bool TryAddGuildOrganization(ref GuildInfo info,
                                               HtmlDocument html) {
      try {
        HtmlNode guildOrgNode = html.DocumentNode.SelectSingleNode(
            "//table[@class=\"rank_table\"]/tbody");
        ...
        foreach (HtmlNode item in guildOrgNode.Descendants("tr")) {
          ...
          string name =
              item.SelectSingleNode("./td[2]/dl/dt/a").InnerText.Trim();
    ...
    public static bool IsNextPageExist(HtmlDocument html) {
      try {
        HtmlNode node =
            html.DocumentNode.SelectSingleNode("//span[@class=\"cm_next\"]/a");
    ...
    private static GuildPosition
    ParseGuildPosition(string position) => position switch {
        "마스터" => GuildPosition.Owner, "부마스터" => GuildPosition.Staff,
    ...
    private static(int gid, WorldID wid) ParseValueFromGuildLink(string url) {
      ...
      foreach (Match match in matches) {
        ...
        switch (param [0]
                    .Remove(0, 1)) {
        case "wid":
          wid = Enum.IsDefined(typeof(WorldID), tmp) ? (WorldID) tmp
                                                     : WorldID.Unknown;
          break;
