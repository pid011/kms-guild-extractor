...
namespace KMSGuildExtractor.Core.Tests {
  ...
  public class GuildDetailParsingTest : WebParsingTest {
    ...
    public void GuildDetailParsingTest1() {
      ...
      Task<GuildInfo> task = Guild.GetGuildDetailAsync(
          new GuildInfo("고잉메리호", WorldID.Reboot, 2210), tokenSource.Token);
      Task.WaitAll(task);
    ...
    public void GuildDetailParsingTest2() {
      var tokenSource = new CancellationTokenSource();
      Task<GuildInfo> task = Guild.GetGuildDetailAsync(
          new GuildInfo("훈장교", WorldID.Scania, 241077), tokenSource.Token);
      Task.WaitAll(task);

      Assert.IsTrue(task.Result.Users.Any(
          u => u.Name == "신남" && u.Position == GuildPosition.Owner));
